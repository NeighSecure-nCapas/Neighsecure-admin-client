...
const routes = [
    ...
    {name: "Hogares", route: "#", icon: hogaresIcon},
    {name: "Residentes", route: "#", icon: residentesIcon},
    {name: "Visitantes", route: "#", icon: visitantesIcon},
    {name: "Entradas", route: "#", icon: entradasIcon},
...
export default function Header() {
    ...
    return (
        <header>
            <nav
                className="sticky top-0 bg-white xl:bg-primaryColor left-0 z-50 flex w-screen flex-row items-center justify-between gap-10 self-start px-12 py-4 xl:px-4 lg:w-64 xl:h-screen xl:flex-col xl:py-8"
            >
                <div className={"hidden xl:flex xl:flex-col w-full h-full justify-around"}>
                    <div
                        className={'flex flex-col justify-center items-center gap-4 text-lg text-white'}>
                    ...
                    <div>
                        <h4 className={"text-white px-4 font-normal"}>
                            {"Menu"}
                        </h4>
                ...
                <motion.div
                    ...
                    <ul
                        className="my-10 flex flex-col items-center gap-10 xl:items-start xl:justify-center xl:pl-64">

                        {routes.map((route) => (
                            <motion.li
                                key={route.name}
                                variants={itemsVariants}
                                whileHover={{x: 3, transition: {duration: 1, type: "spring", ease: "easeInOut"}}}
...
function HeaderItem({icon, text, route, isSelected = false}: HeaderItemProps) {
    const navigate = useNavigate();
    return(
        <Button
            onClick={() => {navigate(route)}}
            variant={'ghost'}
            className={`icon flex flex-row gap-4 py-6 px-4 items-center justify-start bg-primaryColor hover:bg-white hover:text-primaryColor text-white ${!isSelected ? " " : "bg-white text-primaryColor"} `}>
